...
// Botão flutuante de WhatsApp — aparece após pequeno scroll, deep link
// contextual genérico (a versão por-cotação injeta contexto via lib/whatsapp).
export function WhatsappFab() {
  const [visivel, setVisivel] = useState(false);

  useEffect(() => {
    const onScroll = () => setVisivel(window.scrollY > 280);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });
    return () => window.removeEventListener("scroll", onScroll);
  }, []);

  const href = whatsappLink({ origem: "botao-flutuante" });
  ...
  return (
    <a
      ...
      className={`fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full bg-[#25D366] px-4 py-3 text-white shadow-xl shadow-[#25D366]/40 transition-all duration-300 hover:brightness-105 ${
        visivel
